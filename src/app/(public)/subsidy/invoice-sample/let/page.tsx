...
export const metadata: Metadata = {
  title: "適格請求書 出力サンプル（LET版）｜受発注Lシステム｜IT導入補助金 申請書類",
  description:
    "受発注Lシステムが出力する適格請求書（インボイス）のサンプル（株式会社LET 申請用）。登録番号・取引年月日・税率別合計・税率別消費税額・適用税率等の必要記載事項を網羅。",
}

/* Sample data — dummy values for the qualified-invoice output template */
const sample = {
    ...
    <>
      ...
      <div className="print-hide sticky top-14 z-10 border-b border-slate-200 bg-white/95 backdrop-blur">
        <div className="mx-auto max-w-4xl flex items-center justify-between px-6 py-3">
          <Link href="/subsidy" className="text-sm text-slate-600 hover:text-slate-900">
            ← 申請資料一覧に戻る
          </Link>
          <PrintButton />
        </div>
      ...
      <div className="print-hide mx-auto max-w-4xl px-6 pt-8">
        <div className="rounded border border-slate-200 bg-slate-50 p-4 text-sm leading-relaxed">
          <p className="font-bold mb-1">本ページについて（LET 申請版）</p>
          <p>
            本ページは、<strong>株式会社LET（開発メーカー兼 IT導入支援事業者）</strong>
            による IT導入補助金 申請用に作成された、受発注Lシステムが出力する
            <strong>適格請求書（インボイス）のサンプル</strong>です。
            デジタル化・AI導入補助金2026 インボイス枠の申請時に求められる
      ...
      {/* Qualified-invoice template (printable) */}
      <div className="mx-auto max-w-[210mm] p-4 sm:p-8 font-sans text-sm text-gray-900">
        ...
        <div className="border-t border-gray-300 pt-3 text-xs text-gray-500 text-center">
          <p>本サンプルは受発注Lシステムの出力レイアウトを示すためのダミーデータです。</p>
          <p>© 2026 受発注Lシステム / デジタル化・AI導入補助金 2026 申請添付書類</p>
        </div>
      </div>
    </>
  )
