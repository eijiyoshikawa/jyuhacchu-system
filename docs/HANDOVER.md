...
| 項目 | 状態 |
|---|---|
| 申請状況 | ❌ **不採択（2026-08、理由の記載なし）**。ITツール名を「電子取引Lシステム」→**「電子取引くん」**に変更して別ITツールとして再申請する方針。⚠️ 価格説明資料の「導入事例・実績」が要領の求める**過去の実績**になっていない点が最有力の不採択理由（Playbook §11-B）。名称変更だけでは再度不採択の可能性が高い |
...
| 資料 | LET版 `/transact/subsidy/*` ／ TX.企画版 `/transact/subsidy/*/tx`（各6書類＋インデックス。機能説明資料は Fig.1〜10 全図に電子取引Lブランドの画面画像入り）。**資料本文は `_components/transact-*-document.tsx` を両版で共有**し、IT導入支援事業者名のみ props で差し替えるため、不備対応の修正は 1 箇所で両版に反映される |
...
- [x] 「電子取引くん」の名称重複確認 → **既存ツールなし**（2026-09-04 ユーザー確認）
- [x] 導入事例を実績ベースに刷新（想定事例を削除し株式会社Cometaを記載）
- [x] 標準販売価格の引き下げ（300万 → 240万円）
...
| 最小販売価格（税抜） | `1200000` |
| 機能説明資料の総ページ数 | **30ページ** |
| Fig.11 アカウント利用状況 | **P.23** |
| Fig.12 電子取引アーカイブ | **P.24** |
| 業務フロー図の該当ページ | **P.25〜26**（旧 P.23〜24） |
| 提出書類 | **7点**（料金表の別添を追加） |

価格設定の内訳（255字制限・244字）等のテンプレは
`docs/VENDOR_APPLICATION_GUIDE.md` に最新版を収録。

### E. 不備通知が来た場合

- [ ] 通知文をそのまま Claude に貼付 → Playbook §8・§11-B と照合して修正 → PR → マージ → デプロイ
- [ ] ⚠️ 別紙1(2)「**資料内で前回からの修正箇所を明記すること**」を必ず実施
      （過去4回の不備対応でこれを怠っていた）
